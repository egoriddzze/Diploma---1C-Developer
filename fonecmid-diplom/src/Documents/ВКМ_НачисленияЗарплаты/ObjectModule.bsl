...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	РасчитатьИСформироватьДополнительныеНачисленияПроцентЗаРаботы();
	...
	Запрос.Текст =
		...
		|	ВТосн.РезультатОсн + ВТдоп.РезультатДоп - ВТуд.РезультатУд КАК ИтогСумма
		|ИЗ
		|	ВТосн КАК ВТосн
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТуд КАК ВТуд
		|		ПО ВТосн.Сотрудник = ВТуд.Сотрудник
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТдоп КАК ВТдоп
		|		ПО ВТосн.Сотрудник = ВТдоп.Сотрудник";
...
Процедура РасчитатьИСформироватьДополнительныеНачисленияПроцентЗаРаботы()
	...
	Запрос.Текст =
		...
		|		ПО ВТ.Сотрудник = ВКМ_ВыполненныеСотрудникомРаботыОбороты.Сотрудник
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ВКМ_УсловияОплатыСотрудников.СрезПоследних(&Дата, Сотрудник В
		|			(ВЫБРАТЬ
		|				ВКМ_НачисленияЗарплатыНачисления.Сотрудник КАК Сотрудник
		|			ИЗ
	...
	Запрос.УстановитьПараметр("ПроцЗаРабот", ПланыВидовРасчета.ВКМ_ДополнительныеНачисления.ПроцентЗаВыполненныеРаботы);
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Если ТипЗнч(ВыборкаДетальныеЗаписи.ВидРасчета) = Тип("ПланВидовРасчетаСсылка.ВКМ_ДополнительныеНачисления") Тогда
			...
			Если ВыборкаДетальныеЗаписи.ВидРасчета = ПланыВидовРасчета.ВКМ_ДополнительныеНачисления.Премия Тогда
				Сообщение = Новый СообщениеПользователю();
				Сообщение.Текст = "Начисление премии производится в документе ""Начисление фиксированной премии""";
				Сообщение.Сообщить();
				Возврат;
			КонецЕсли;
			...
			Движение.ВидРасчета = ВыборкаДетальныеЗаписи.ВидРасчета;
			Движение.Сотрудник = ВыборкаДетальныеЗаписи.Сотрудник;
			Движение.Результат = (ВыборкаДетальныеЗаписи.СуммаКОплатеОборот/100) * ВыборкаДетальныеЗаписи.ПроцентОтРабот;
			Движение.КатегорияСотрудника = ВыборкаДетальныеЗаписи.КатегорияСотрудника;
...
Процедура РасчитатьНДФЛ()
	...
	Запрос.Текст =
		...
		|			Регистратор = &Ссылка
		|		И ВидРасчета = &ВидРасчетаНДФЛ) КАК ВКМ_УдержанияБазаВКМ_ДополнительныеНачисления
